  ...
  Eigen::MatrixXd & W)
{
  ...
  vector<int> P;
  // read TGF as C,P,BE
  bool tgf_success = read_TGF(tgf_filename,C,E,P,BE,CE,PE);
  if(!tgf_success)
  {
    return false;
  }
  ...
  for(int i = 0;i<(int)P.size();i++)
  {
    ...
    offset(1) = C[P[i]][1];
    offset(2) = C[P[i]][2];
    Bone * b = *BR.insert(BR.end(),new Bone(NULL,offset));
    b->set_wi(i);
  }
  ...
  for(int i = 0;i<(int)BE.size();i++)
  {
    ...
    {
      // new root
      parent = *BR.insert(BR.end(),new Bone(NULL,pi_offset));
    ...
    Bone * c = new Bone(parent,ci_offset);
    // add to index2bone map
    index2bone[ci] = c;
    c->set_wi(P.size() + i);
  }
