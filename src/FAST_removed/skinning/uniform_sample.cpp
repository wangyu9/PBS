...
static void uniform_sample_mesh_vertices(
  ...
  Eigen::MatrixXd & WS)
{
  ...
  uniform_sample_mesh_vertices(W,k,push,S);
  verbose("Lap: %g\n",get_seconds()-start);
  ...
  // face its on
  vector<vector<Vec3> > sample_barys; sample_barys.resize(k);
  ...
  while(radius > min_radius)
  {
    ...
    for(int i = 0;i < k;i++)
    {
      ...
      Vec3 bary(sample_barys[i][cur_maxmin[i]]);
...
//		  double rv = fgenrand();
    ...
    for(int i = 0;i < k;i++)
    {
      ...
      for(int s = 0;s<(int)sample_faces[i].size();s++)
      {
        ...
        Vec3 sample_bary = sample_barys[i][s];
        ...
        for(int j = 0;j < k;j++)
        {
          ...
          }else
          {
            ...
            Vec3 seed_j_bary(sample_barys[j][cur_maxmin[j]]);
...
void uniform_sample_mesh_vertices(
  const Eigen::MatrixXd & OW,
  const int k, 
  const double push,
  Eigen::VectorXi & S)
