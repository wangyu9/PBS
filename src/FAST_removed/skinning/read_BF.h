...
inline bool read_BF(const char * file_name,std::vector<Bone*> & BR);
...
inline bool read_BF(const char * file_name,std::vector<Bone*> & BR)
{
  ...
  int lineno = 1;
  int max_wi = -1;
  // read lines until seeing end of file
  while(fgets(line,MAX_LINE_LENGTH,fp)!=NULL)
  {
    ...
    int dof_type = -1; // 0 - NORMAL_DOF, 1 - FREE_DOF, 2 - FIXED_DOF
    ...
    {
      // ROOT
      b = *BR.insert(BR.end(),new Bone(NULL,offset));
    ...
    if(numvar >= 27)
    {
      ...
      {
        case 0:
          b->tip_dof_type = NORMAL_DOF;
          break;
        case 1:
          b->tip_dof_type = FREE_DOF;
          break;
        case 2:
          b->tip_dof_type = FIXED_DOF;
          break;
        default:
