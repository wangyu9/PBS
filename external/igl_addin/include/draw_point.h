#ifndef IGL_ADDIN_DRAW_POINT_H
#define IGL_ADDIN_DRAW_POINT_H
...
inline void draw_point(double x, double y, double z, double r, bool selected, float rr=0, float gg=1, float bb=0)
{
	// notice that the Maximum radius is limited by the *Hardware*, for my intel hd 4600 is limited, my GTM 880 is fine with that.
  ...
  // White outline
  glColor4f(1,1,1,old_color[3]);
  glPointSize(2*r);
  glBegin(GL_POINTS);
  glVertex3d(x,y,z);
  glEnd();
  // Black outline
  glColor4f(0,0,0,old_color[3]);
  glPointSize(1.6*r);
  glBegin(GL_POINTS);
  glVertex3d(x,y,z);
  glEnd();
