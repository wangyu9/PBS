...
namespace igl
{
	class PreRotateWidgetGLSettings{
	public:
		int l;
		int dt;
		double lw;
  ...
  class RotateWidget
  {
    ...
    public:
	  ...
	  // wangyu added this in order to break into draw
	  inline void draw_pre(PreRotateWidgetGLSettings& ps) const;
	  inline void draw_mid() const;
	  inline void draw_post(const PreRotateWidgetGLSettings& ps) const;
  };
}
...
{
	PreRotateWidgetGLSettings ps;
	draw_pre(ps);
	draw_mid();
	draw_post(ps);
}

inline void igl::RotateWidget::draw_pre(PreRotateWidgetGLSettings& ps) const
{
	using namespace Eigen;
	using namespace std;
	using namespace igl;
	//int l, dt;
	glGetIntegerv(GL_LIGHTING, &ps.l);
	glGetIntegerv(GL_DEPTH_TEST, &ps.dt);
	//double lw;
	glGetDoublev(GL_LINE_WIDTH, &ps.lw);
...
inline void igl::RotateWidget::draw_mid() const
{
	using namespace Eigen;
	using namespace std;
	using namespace igl;
...
inline void igl::RotateWidget::draw_post(const PreRotateWidgetGLSettings& ps) const
{
	glPopMatrix();//glMultMatrixd(Affine3d(rot).matrix().data());

	glPopMatrix();//glTranslated(pos(0), pos(1), pos(2));

	glLineWidth(ps.lw);
	(ps.l ? glEnable(GL_LIGHTING) : glDisable(GL_LIGHTING));
	(ps.dt ? glEnable(GL_DEPTH_TEST) : glDisable(GL_DEPTH_TEST));
}
