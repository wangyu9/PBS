...
#include <igl/OpenGL_convenience.h>// wangyu modified
#include <igl/draw_point.h>// wangyu modified
#include <igl/material_colors.h>// wangyu modified
  ...
  const float scale)
{
  ...
  for(int i = 0;i<3;i++)
  {
    switch(i)
    {
      case 0: glColor3fv(WHITE);      glLineWidth(10 * scale); break;
	  case 1: glColor3fv(BLACK);      glLineWidth(6 * scale); break;
	  case 2: glColor3fv(line_color); glLineWidth(4 * scale); break;
    }
...
template void igl::draw_skeleton_vector_graphics<Eigen::Matrix<double, -1, -1, 0, -1, -1>, Eigen::Matrix<int, -1, -1, 0, -1, -1>, Eigen::Matrix<double, -1, -1, 0, -1, -1> >
