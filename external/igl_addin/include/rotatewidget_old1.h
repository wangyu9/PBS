...
#ifndef IGL_ADDIN_ROTATE_WIDGET_H
#define IGL_ADDIN_ROTATE_WIDGET_H
...
public:
	...
	inline bool on_meridian(
		const Eigen::Vector3d & hit,
		const Eigen::Quaterniond & rot,
		const Eigen::Quaterniond & m,
		const double thresh,
		Eigen::Vector3d & pl_hit); //added by wangyu


	void draw_circle(const bool cull) const;
};
...
inline void RotateWidget::glColor4fv(
    const bool a,
...
inline Eigen::Quaterniond RotateWidget::axis_q(const int a)
{
  assert(a<3 && a>=0);
  /*wangyu
  const Eigen::Quaterniond axes[3] = {
	...
	*/
  Eigen::Quaterniond axes[3];
  axes[0] = Eigen::Quaterniond(Eigen::AngleAxisd(igl::PI*0.5, Eigen::Vector3d(0, 1, 0)));
  axes[1] = Eigen::Quaterniond(Eigen::AngleAxisd(igl::PI*0.5, Eigen::Vector3d(1, 0, 0)));
  axes[2] = Eigen::Quaterniond::Identity();
  return axes[a];
}

inline Eigen::Vector3d RotateWidget::view_direction(const int x, const int y)
{
...
inline Eigen::Vector3d RotateWidget::view_direction(const Eigen::Vector3d & pos)
{
  using namespace Eigen;
  using namespace igl;
...
inline RotateWidget::RotateWidget():
  pos(0,0,0),
...
inline Eigen::Vector3d RotateWidget::unproject_onto(
  const int x, 
  const int y) const
{
  using namespace Eigen;
...
inline bool RotateWidget::intersect(
  const int x, 
  const int y,
  Eigen::Vector3d & hit) const
{
...
inline double RotateWidget::unprojected_inner_radius() const
{
  using namespace Eigen;
  using namespace igl;
...
inline bool RotateWidget::on_meridian(
	const Eigen::Vector3d & hit,
	const Eigen::Quaterniond & rot,
	const Eigen::Quaterniond & m,
	const double thresh,
	Eigen::Vector3d & pl_hit)
{
	...
	pl_hit += pos;
	return (igl::project(pl_hit).head(2) - igl::project(hit).head(2)).norm() < 2 * thresh;
};

void RotateWidget::draw_circle(const bool cull) const
{
	Eigen::Vector3d view = view_direction(pos).normalized();
	glBegin(GL_LINES);
	const double th_step = (2.0*igl::PI / 100.0);
	for (double th = 0; th < 2.0*igl::PI + th_step; th += th_step)
	{
		Eigen::Vector3d a(cos(th), sin(th), 0.0);
		Eigen::Vector3d b(cos(th + th_step), sin(th + th_step), 0.0);
		if (!cull || (0.5*(a + b)).dot(view) < igl::FLOAT_EPS)
		{
			glVertex3dv(a.data());
			glVertex3dv(b.data());
		}
...
//	const Eigen::Vector3d & hit,
//	const Eigen::Quaterniond & rot,
//	const Eigen::Quaterniond & m,//changed by wangyu
//	Eigen::Vector3d & pl_hit)
//{
//	// project onto rotate plane
//	pl_hit = hit - pos;
...
inline bool RotateWidget::down(const int x, const int y)
{
  using namespace Eigen;
  using namespace igl;
  using namespace std;
  ...
  }else if(r < outer_radius_on_screen/outer_over_inner+thresh*0.5)
  {
    ...
    //  const Quaterniond & m,//changed by wangyu
    ...
    {
      Vector3d pl_hit;
	  Quaterniond m = Quaterniond(axis_q(a));
      if(on_meridian(hit,rot,m,thresh,pl_hit))
      {
...
inline bool RotateWidget::drag(const int x, const int y)
{
  using namespace igl;
  using namespace std;
  using namespace Eigen;
...
inline bool RotateWidget::up(const int /*x*/, const int /*y*/)
{
...
inline bool RotateWidget::is_down() const
{
  return down_type != DOWN_TYPE_NONE;
}

inline void RotateWidget::draw() const
{
  using namespace Eigen;
  using namespace std;
  using namespace igl;
...
inline void RotateWidget::draw_guide() const
{
  using namespace Eigen;
  using namespace std;
  using namespace igl;
