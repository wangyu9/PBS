     ...
     )
{
  ...
  mexErrMsgIdAndTxt("MATLAB:mexcpp:nargin", 
      "lim_solver_mex requires 15 input arguments.");
  } 
  else if (nlhs != 1) {
  ...
  const mxArray* tolerance_mx = prhs[8];
  const mxArray* maxIteration_mx = prhs[9];
  const mxArray* findLocalMinima_mx = prhs[10];
  const mxArray* enableOuput_mx = prhs[11];
  const mxArray* enableAlphaUpdate_mx = prhs[12];
  const mxArray* beta_mx = prhs[13];
  const mxArray* eps_mx = prhs[14];
  ...
  vector<vector<double> > tolerance_VV = readMatrix<double>(tolerance_mx);
  vector<vector<double> > maxIteration_VV = readMatrix<double>(maxIteration_mx);
  vector<vector<double> > findLocalMinima_VV = readMatrix<double>(findLocalMinima_mx);
  ...
  double tolerance = tolerance_VV[0][0];
  int maxIteration = maxIteration_VV[0][0];

  bool findLocalMinima = findLocalMinima_VV[0][0];
  ...
  if(false)
  {
    ...
    cerr << tolerance << endl;
    cerr << "---------" << endl;
    cerr << "maxIteration:" << endl;
    cerr << maxIteration << endl;
    cerr << "---------" << endl;
    cerr << "findLocalMinima:" << endl;
    cerr << findLocalMinima << endl;
    cerr << "---------" << endl;
  ...
  int r = ComputeLIM(
    vertices, initialVertices, elements,
    borderVertices, gradients, constraintMatrix, constraintTargets,
    energyType, tolerance, maxIteration, findLocalMinima,
    enableOuput, enableAlphaUpdate, beta, eps);
