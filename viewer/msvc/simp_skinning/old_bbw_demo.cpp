...
int main(int argc, char * argv[])
{
	...
	std::string mesh_filename = "examples/brick.tgf";//argc[1];
	std::string	skel_filename = "examples/brick.tgf";//argc[2];
	...
	if(!load_skeleton_from_file(skel_filename,C,P,BE,CE))
	{
		return 1;
	}
...
//#endif
	...
	// Save output
	save_output(mesh_filename,skel_filename,V,F,VV,TT,FF,W);
	return 0;
}
